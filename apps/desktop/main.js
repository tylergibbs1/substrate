...
let serverProc = null;
let mainWindow = null;
let updater = null; // electron-updater's autoUpdater, loaded lazily in production
...
// Auto-update from the GitHub Release. No-ops in dev. macOS needs a signed build
// (now satisfied — Squirrel.Mac verifies the signature); Windows updates work
// unsigned but without that signature check until a cert is added. We forward
// electron-updater's lifecycle to the renderer so the UI can surface a ready
// update (a quiet "Restart to update" pill) and a manual check.
async function setupAutoUpdate() {
  if (isDev) return;
  try {
    const m = await import("electron-updater");
    updater = m.autoUpdater ?? m.default?.autoUpdater ?? null;
  } catch {
    updater = null;
  }
  if (!updater) return;
  ...
  const send = (state) => {
    if (mainWindow && !mainWindow.isDestroyed()) mainWindow.webContents.send("substrate:update", state);
  };
  updater.autoDownload = true;
  updater.on("checking-for-update", () => send({ status: "checking" }));
  updater.on("update-available", (info) => send({ status: "downloading", version: info?.version }));
  updater.on("update-not-available", () => send({ status: "uptodate" }));
  updater.on("download-progress", (p) => send({ status: "downloading", percent: Math.round(p?.percent ?? 0) }));
  updater.on("update-downloaded", (info) => send({ status: "downloaded", version: info?.version }));
  updater.on("error", (e) => send({ status: "error", error: String(e?.message ?? e) }));

  // Initial background check once the window is up; errors surface via the event.
...
app.whenReady().then(async () => {
  ...
  startServer();
  // Update IPC: the renderer reads the running version, pulls a manual check, and
  // triggers install-and-restart. All no-op safely if the updater didn't load
  // (dev, or electron-updater missing) so the renderer's calls never hang.
  ipcMain.handle("substrate:version", () => app.getVersion());
  ipcMain.handle("substrate:checkForUpdates", () => {
    updater?.checkForUpdates().catch(() => {});
  });
  ipcMain.handle("substrate:restartToUpdate", () => {
